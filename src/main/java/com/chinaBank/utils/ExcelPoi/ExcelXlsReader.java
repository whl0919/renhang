...
import java.util.Map;
 ...
 public class ExcelXlsReader implements HSSFListener {
	 private ExcelReaderUtil eUtil;
	 private String sheetNameNow="";//当前的页签名称
     //存储所有行记录的容器
     private List<Map<String,Object>> rowList = new LinkedList();
     public  List<ExcelBaseTabData> excelBaseTabDatas;
	 public static String uuid = "",ZQ = "",org = "",user = "";
     private int minColums = -1;
 
     private POIFSFileSystem fs;
//     private List<Map<String,Object>> container =  new LinkedList<>();
     /**
      * 总行数
      */
      ...
      * @throws Exception
      */
     public int process(String fileName,ExcelReaderUtil excelReaderUtil) throws Exception {
    	 eUtil = excelReaderUtil;
         filePath = fileName;
    	 ...
    	 m.put("sheetName", sheetNameNow);
    	 m.put("rowList", rowList);
    	 eUtil.writeDataBase(m,excelBaseTabDatas,uuid,ZQ,org,user);//写数据
...
//            	 dataMap.put("curRow", curRow);
            	 dataMap.put("cellList", cellList);
            	 if("" == sheetNameNow) {//第一行数据
            		 rowList.add(0, null);
            		 rowList.add(1, null);
            		 sheetNameNow = sheetName;
            		 rowList.add(dataMap);
            	 }else {
            		 if(sheetNameNow.equals(sheetName)) {//相同
            			 rowList.add(dataMap);
            			 if(rowList.size() == 5000) {
                        	 ...
                        	 m.put("rowList", rowList);
                        	 try {
                        		 eUtil.writeDataBase(m,excelBaseTabDatas,uuid,ZQ,org,user);
                        		 rowList.clear();
							} catch (Exception e) {
								e.printStackTrace();
							}//写数据
                    	 ...
                    	 try {
                    		 eUtil.writeDataBase(m,excelBaseTabDatas,uuid,ZQ,org,user);
            				 rowList.clear();
                			 sheetNameNow = sheetName;//将新的页签赋给当前
                    		 rowList.add(0, null);
                    		 rowList.add(1, null);
                    		 rowList.add(dataMap);
						} catch (Exception e) {
